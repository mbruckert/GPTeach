...
DOCUMENTATION:
{context}
Always use ArrowVectorStore instead of VectorStore for vector fields. Whenever you write ANYTHING in MathTex, ALWAYS start and end the string with a dollar sign. So instead of MathTex(equation), use MathTex($equation$). NEVER try to highlight individual elements of Tex equations. Instead, put a colored box around them. ALWAYS scale text elements by 0.8. NEVER attempt to reference ANY outside asset (e.g. image) as you will DIE
Instead of ShowCreation, ALWAYS use Create. When creating a Table, DO NOT specify row_dim, col_dim, row_alignments, or col_alignments
